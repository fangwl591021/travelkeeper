...
function isSafeTenantSlug(value) {
...
export function isLegacyCustomerCompatRequest(request) {
  ...
  if (!tenant || !isSafeTenantSlug(tenant)) return false;
  const url = new URL(request.url);
  const path = url.pathname.replace(/\/+$/, '') || '/';
  if (path === '/api/orders/status' && request.method === 'GET') return true;
  if (tenant === 'demo') return false;
...
async function listLegacyCustomers(request, env) {
  ...
  const ownerUid = canReadAll
    ? (requestedUid && requestedUid !== context.userUid ? requestedUid : '')
    : context.userUid;
  if (ownerUid) {
    where += ' AND owner_uid = ?';
    binds.push(ownerUid);
...
async function legacyOrderCreate(request, env, legacyWorker) {
  const body = await request.json().catch(() => ({}));
  const context = await requireContext(request, env, ['platform_admin', 'tenant_admin', 'editor', 'sales', 'finance', 'support', 'member'], body);
  const salesScoped = ['sales', 'editor'].includes(context.role);
  const requestedDistributorUid = cleanText(body.distributor_uid || body.distributorUid, 100);
  const requestedInviteCode = cleanText(body.invite_code || body.inviteCode, 100);
  if (salesScoped && ((requestedDistributorUid && requestedDistributorUid !== context.userUid) || requestedInviteCode)) {
    throw new Error('TENANT_ACCESS_DENIED');
  }
  ...
  const translated = new Request(url.toString(), {
    ...
    body: JSON.stringify({
      ...
      distributor_uid: salesScoped ? context.userUid : (body.distributor_uid || body.distributorUid),
      invite_code: salesScoped ? '' : (body.invite_code || body.inviteCode),
