...
function publicAssignee(row) {
  return row.assigned_to_uid ? { user_uid: row.assigned_to_uid, role: row.assigned_role || '' } : null;
}
...
export function isTenantLineMonitorApiRequest(request) {
  ...
  return path === '/api/v2/line/threads' || /^\/api\/v2\/line\/threads\/[^/]+(?:\/messages|\/assignment|\/claim|\/read)?$/.test(path);
}
