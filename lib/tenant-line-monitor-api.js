...
import { DEFAULT_SLA_SETTINGS, acknowledgeBreach, calculateSla, closeWaitingCycle, normalizeSlaSettings, pauseWaitingCycle, resumeWaitingCycle } from './tenant-line-sla.js';
...
function flagEnabled(env, name, defaultEnabled = true) {
...
async function getSlaSettings(env, tenantSlug) {
  ...
  return { ...normalizeSlaSettings(row || DEFAULT_SLA_SETTINGS), ...(row ? {
...
function validPriority(value) {
  ...
  if (!['low', 'normal', 'high', 'urgent'].includes(valueText)) throw new Error('INVALID_LINE_PRIORITY');
  return valueText;
}
...
function threadView(row, settings = DEFAULT_SLA_SETTINGS, env = null) {
  const sla = flagEnabled(env, 'TENANT_LINE_SLA_ENABLED') ? calculateSla(row, settings) : { sla_status: 'not_applicable', waiting_seconds: 0, remaining_seconds: 0, overdue_seconds: 0, sla_breached_at: row.sla_breached_at || '' };
  return {
    id: row.id,
    ...
    sla_remaining_seconds: Number(row.sla_remaining_seconds || 0),
    last_customer_wait_seconds: Number(row.last_customer_wait_seconds || 0),
    total_customer_wait_seconds: Number(row.total_customer_wait_seconds || 0),
    response_count: Number(row.response_count || 0),
    sla_type: row.first_response_at ? 'followup' : 'first_response',
    ...sla,
  };
}
...
async function listThreads(request, env) {
  ...
  const waitingOnly = url.searchParams.get('waiting_only') === 'true';
  const breachedOnly = url.searchParams.get('breached_only') === 'true';
  const slaFilter = text(url.searchParams.get('sla_status'), 20);
  const priorityFilter = text(url.searchParams.get('priority'), 20);
  ...
  if (waitingOnly) where.push("COALESCE(t.waiting_since, '') <> ''");
  if (priorityFilter && priorityFilter !== 'all') { where.push('t.priority = ?'); args.push(validPriority(priorityFilter)); }
  ...
  const rows = await env.DB.prepare(`
    ...
    LIMIT ?
  `).bind(...args, limit).all();
  const settings = await getSlaSettings(env, ctx.tenantSlug);
  const rawRows = rows.results || [];
  await confirmBreaches(env, ctx, rawRows, settings);
  let data = rawRows.map(row => threadView(row, settings, env));
  if (slaFilter && slaFilter !== 'all') data = data.filter(row => row.sla_status === slaFilter);
  if (breachedOnly) data = data.filter(row => row.sla_status === 'breached');
  ...
  return json({ success: true, data: sortThreads(data), tenant_slug: ctx.tenantSlug, role: ctx.role, sla_settings: normalizeSlaSettings(settings) });
}
async function listMessages(request, env, threadId) {
  ...
  return json({ success: true, data: { thread: threadView(thread, DEFAULT_SLA_SETTINGS, env), messages: (rows.results || []).map(messageView) } });
}
...
async function auditSettings(env, ctx, before = {}, after = {}) {
  await env.DB.prepare(`
    INSERT INTO audit_logs (
      id, tenant_slug, actor_uid, action, target_type, target_id,
      before_json, after_json, request_id, created_at
    ) VALUES (?, ?, ?, 'tenant.line.sla_settings.update', 'tenant_line_sla_settings', ?, ?, ?, ?, datetime('now'))
  `).bind(
    `AUDIT-LINE-SLA-${(await sha256Hex(`${ctx.tenantSlug}:sla:${Date.now()}`)).slice(0, 32).toUpperCase()}`,
    ctx.tenantSlug,
    ctx.userUid,
    ctx.tenantSlug,
    JSON.stringify(before || {}),
...
async function getSettings(request, env) {
  const ctx = await context(request, env);
  const settings = await getSlaSettings(env, ctx.tenantSlug);
  return json({ success: true, data: settings, tenant_slug: ctx.tenantSlug, role: ctx.role });
}

async function patchSettings(request, env) {
  const ctx = await adminContext(request, env);
  const before = await getSlaSettings(env, ctx.tenantSlug);
  const body = await request.json().catch(() => ({}));
  const next = normalizeSlaSettings({ ...before, ...body });
  await env.DB.prepare(`
    INSERT INTO tenant_line_sla_settings (
      tenant_slug, first_response_sla_minutes, followup_response_sla_minutes, due_soon_percentage,
      pause_sla_on_pending, created_by, updated_by, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
    ON CONFLICT(tenant_slug) DO UPDATE SET
      first_response_sla_minutes = excluded.first_response_sla_minutes,
      followup_response_sla_minutes = excluded.followup_response_sla_minutes,
      due_soon_percentage = excluded.due_soon_percentage,
      pause_sla_on_pending = excluded.pause_sla_on_pending,
      updated_by = excluded.updated_by,
      updated_at = datetime('now')
  `).bind(
    ctx.tenantSlug,
    next.first_response_sla_minutes,
    next.followup_response_sla_minutes,
    next.due_soon_percentage,
    next.pause_sla_on_pending,
    ctx.userUid,
    ctx.userUid,
  ).run();
  const after = await getSlaSettings(env, ctx.tenantSlug);
  await auditSettings(env, ctx, before, after);
  return json({ success: true, data: after, tenant_slug: ctx.tenantSlug, role: ctx.role });
}
...
function sortThreads(data) {
  return data.sort((a, b) => {
    const breach = Number(b.sla_status === 'breached') - Number(a.sla_status === 'breached');
    if (breach) return breach;
    const priority = priorityRank(a.priority) - priorityRank(b.priority);
    if (priority) return priority;
    const dueSoon = Number(b.sla_status === 'due_soon') - Number(a.sla_status === 'due_soon');
    if (dueSoon) return dueSoon;
    const unread = Number(b.unread_count > 0) - Number(a.unread_count > 0);
    if (unread) return unread;
    return String(a.waiting_since || '9999').localeCompare(String(b.waiting_since || '9999'))
      || String(b.last_message_at || '').localeCompare(String(a.last_message_at || ''));
  });
}

async function confirmBreaches(env, ctx, rows, settings) {
  for (const row of rows) {
    const patch = acknowledgeBreach(row, settings);
    if (!patch) continue;
    await env.DB.prepare(`
      UPDATE tenant_crm_threads
      SET sla_status = 'breached', sla_breached_at = ?, updated_at = datetime('now')
      WHERE tenant_slug = ? AND id = ? AND COALESCE(sla_breached_at, '') = ''
    `).bind(patch.sla_breached_at, ctx.tenantSlug, row.id).run();
    await auditThread(env, ctx, 'tenant.line.sla.breach_confirmed', row.id, { sla_breached_at: '' }, patch);
    row.sla_status = 'breached';
    row.sla_breached_at = patch.sla_breached_at;
  }
}
async function updateThread(request, env, threadId) {
  ...
  const statusTarget = nextQueue || nextStatus;
  const settings = await getSlaSettings(env, ctx.tenantSlug);
  let slaPatch = null;
  let slaAudit = '';
  if (statusTarget === 'closed') {
    slaPatch = closeWaitingCycle(thread);
    slaAudit = 'tenant.line.sla.close';
  } else if (statusTarget === 'pending') {
    slaPatch = pauseWaitingCycle(thread, settings);
    slaAudit = slaPatch ? 'tenant.line.sla.pause' : '';
  } else if (['open', 'unassigned'].includes(statusTarget) && thread.sla_paused_at) {
    slaPatch = resumeWaitingCycle(thread, settings);
    slaAudit = slaPatch ? 'tenant.line.sla.resume' : '';
  }
  await env.DB.prepare(`
  ...
  if (slaPatch) {
    await env.DB.prepare(`
      UPDATE tenant_crm_threads
      SET waiting_since = ?, sla_started_at = ?, sla_due_at = ?, sla_paused_at = ?,
          sla_remaining_seconds = ?, sla_status = ?, last_customer_wait_seconds = ?,
          total_customer_wait_seconds = ?, updated_by = ?, updated_at = datetime('now')
      WHERE tenant_slug = ? AND id = ?
    `).bind(
      slaPatch.waiting_since ?? thread.waiting_since ?? '',
      slaPatch.sla_started_at ?? thread.sla_started_at ?? '',
      slaPatch.sla_due_at ?? thread.sla_due_at ?? '',
      slaPatch.sla_paused_at ?? '',
      Number(slaPatch.sla_remaining_seconds ?? thread.sla_remaining_seconds ?? 0),
      slaPatch.sla_status || thread.sla_status || 'not_applicable',
      Number(slaPatch.last_customer_wait_seconds ?? thread.last_customer_wait_seconds ?? 0),
      Number(slaPatch.total_customer_wait_seconds ?? thread.total_customer_wait_seconds ?? 0),
      ctx.userUid, ctx.tenantSlug, threadId,
    ).run();
  }
  ...
  if (slaAudit) await auditThread(env, ctx, slaAudit, threadId, { sla_status: thread.sla_status || '', waiting_since: thread.waiting_since || '' }, slaPatch || {});
  return listMessages(request, env, threadId);
}

async function updatePriority(request, env, threadId) {
  const ctx = await context(request, env);
  const thread = await getThread(env, ctx, threadId);
  if (!canOperate(ctx, thread)) throw new Error('LINE_THREAD_ACCESS_DENIED');
  const body = await request.json().catch(() => ({}));
  ...
  await env.DB.prepare(`
    UPDATE tenant_crm_threads
    SET priority = ?, updated_by = ?, updated_at = datetime('now')
    WHERE tenant_slug = ? AND id = ?
  `).bind(priority, ctx.userUid, ctx.tenantSlug, threadId).run();
  await auditThread(env, ctx, 'tenant.line.thread.priority_change', threadId, { priority: thread.priority || 'normal' }, { priority });
  const settings = await getSlaSettings(env, ctx.tenantSlug);
  const updated = await getThread(env, ctx, threadId);
  return json({ success: true, data: { thread: threadView(updated, settings) }, tenant_slug: ctx.tenantSlug, role: ctx.role });
}
...
async function updateAssignment(request, env, threadId) {
  if (!flagEnabled(env, 'TENANT_LINE_QUEUE_ENABLED')) throw new Error('TENANT_LINE_QUEUE_DISABLED');
  const ctx = await context(request, env);
  ...
  const updated = await getThread(env, ctx, threadId);
  return json({ success: true, data: { thread: threadView(updated, DEFAULT_SLA_SETTINGS, env) } });
}
...
async function createOutboundMessage(request, env, threadId) {
  if (!flagEnabled(env, 'TENANT_LINE_OUTBOUND_ENABLED')) throw new Error('TENANT_LINE_OUTBOUND_DISABLED');
  const ctx = await context(request, env);
  const thread = await getThread(env, ctx, threadId);
  if (!canOperate(ctx, thread)) throw new Error('LINE_THREAD_ACCESS_DENIED');
  ...
  if (duplicate) {
    return json({ success: true, duplicate: true, data: { thread: threadView(thread, settings), message: messageView(duplicate) } });
  }
  ...
  if (finalStatus === 'sent') {
    const closeSla = closeWaitingCycle(thread, new Date(createdAt));
    await env.DB.prepare(`
      UPDATE tenant_crm_threads
      ...
      WHERE tenant_slug = ? AND id = ?
    `).bind(
      closeSla.waiting_since,
      closeSla.sla_started_at,
      closeSla.sla_due_at,
      closeSla.sla_paused_at,
      closeSla.sla_remaining_seconds,
      closeSla.sla_status,
      closeSla.last_customer_wait_seconds,
      closeSla.total_customer_wait_seconds,
      ctx.userUid, ctx.tenantSlug, threadId,
    ).run();
    await env.DB.prepare(`
    ...
    await auditThread(env, ctx, 'tenant.line.sla.response', threadId, {
      waiting_since: thread.waiting_since || '',
      response_count: thread.response_count || 0,
      total_customer_wait_seconds: thread.total_customer_wait_seconds || 0,
    }, closeSla);
  }
  ...
  return json({ success: finalStatus === 'sent', data: { thread: threadView(updatedThread, settings), message: messageView(saved) }, error: errorCode || undefined }, finalStatus === 'sent' ? 200 : statusForError(errorCode, 502));
}

export function isTenantLineMonitorApiRequest(request) {
  const path = new URL(request.url).pathname.replace(/\/+$/, '');
  return path === '/api/v2/line/threads' || path === '/api/v2/tenant/line-agents' || path === '/api/v2/tenant/line-sla-settings' || /^\/api\/v2\/line\/threads\/[^/]+(?:\/messages|\/assignment|\/claim|\/read|\/priority)?$/.test(path);
}

export async function routeTenantLineMonitorApi(request, env) {
  try {
    if (!env.DB) throw new Error('D1_REQUIRED');
    if (!flagEnabled(env, 'TENANT_LINE_MONITOR_ENABLED')) throw new Error('TENANT_LINE_MONITOR_DISABLED');
    const path = new URL(request.url).pathname.replace(/\/+$/, '');
    if (request.method === 'GET' && path === '/api/v2/tenant/line-agents') return await listLineAgents(request, env);
    if (request.method === 'GET' && path === '/api/v2/tenant/line-sla-settings') return await getSettings(request, env);
    if (request.method === 'PATCH' && path === '/api/v2/tenant/line-sla-settings') return await patchSettings(request, env);
    ...
    const priority = path.match(/^\/api\/v2\/line\/threads\/([^/]+)\/priority$/);
    if (request.method === 'PATCH' && priority) return await updatePriority(request, env, decodeURIComponent(priority[1]));
