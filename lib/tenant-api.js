...
function mustScopeItinerariesToOwner(context) {
  return ['sales', 'editor'].includes(context.role) &&
    !hasPermission(context, 'itinerary.read.all');
}
...
async function listItineraries(request, env) {
  ...
  if (scope === 'public') {
    ...
    if (mustScopeItinerariesToOwner(context)) {
      where += ' AND owner_uid = ?';
...
async function getOrder(request, env, orderId) {
  ...
  if (!row) return json({ success: false, error: 'ORDER_NOT_FOUND' }, 404);

  const canRead = canReadAllOrders(context) ||
    row.customer_line_uid === context.userUid ||
    row.distributor_uid === context.userUid;
  if (!canRead) throw new Error('TENANT_ACCESS_DENIED');

  return json({ success: true, data: toTenantOrder(row), tenantSlug: context.tenantSlug });
}
...
export async function routeTenantApi(request, env) {
  ...
  try {
    ...
    const orderMatch = path.match(/^\/api\/v2\/orders\/([^/]+)$/);
    if (request.method === 'GET' && orderMatch) {
      return await getOrder(request, env, decodeURIComponent(orderMatch[1]));
    }
