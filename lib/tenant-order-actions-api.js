...
function normalizeOwnerTransferAudit(row = {}) {
  const before = safeObject(row.before_json);
  const after = safeObject(row.after_json);
  return {
    actor_uid: cleanText(row.actor_uid, 120),
    from_owner_uid: cleanText(before.owner_uid, 120),
    from_owner_name: cleanText(before.owner_name, 200),
    to_owner_uid: cleanText(after.owner_uid, 120),
    to_owner_name: cleanText(after.owner_name, 200),
    request_id: cleanText(row.request_id, 160),
...
async function listCustomerOwnerTransferHistory(request, env, customerId) {
  if (!env.DB) throw new Error('D1_REQUIRED');
  const context = await requireOwnerTransferContext(request, env);
  const customer = await loadCustomer(env, context.tenantSlug, customerId);
  if (!customer) throw new Error('CUSTOMER_NOT_FOUND');

  const result = await env.DB.prepare(`
    SELECT actor_uid, before_json, after_json, request_id, created_at
    FROM audit_logs
    WHERE tenant_slug = ?
      AND action = 'customer.owner.transfer'
      AND target_type = 'customer'
      AND target_id = ?
    ORDER BY created_at DESC
    LIMIT 50
  `).bind(context.tenantSlug, customerId).all();

  return json({
    success: true,
    tenantSlug: context.tenantSlug,
    customer_id: customerId,
    data: (result.results || []).map(normalizeOwnerTransferAudit),
  });
}
...
export function isTenantOrderActionRequest(request) {
  ...
  return /^\/api\/v2\/orders\/[^/]+\/balance-paid$/.test(path) ||
    ...
    /^\/api\/v2\/customers\/[^/]+\/owner-transfer-history$/.test(path);
}
...
export async function routeTenantOrderAction(request, env) {
  ...
  try {
    ...
    const historyMatch = path.match(/^\/api\/v2\/customers\/([^/]+)\/owner-transfer-history$/);
    if (historyMatch && request.method === 'GET') {
      return await listCustomerOwnerTransferHistory(request, env, decodeURIComponent(historyMatch[1]));
    }
