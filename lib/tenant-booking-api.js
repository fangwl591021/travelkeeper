...
async function createBooking(request, env) {
  ...
  let initialRefUid = distributorUid;
  let initialOwnerUid = distributorUid;
  let initialOwnerName = distributorName;

  const priorRefUid = String(existingLineProfile?.ref_uid || '').trim();
  if (priorRefUid) {
    const priorReferrer = await resolveDistributor(env, tenantSlug, { distributor_uid: priorRefUid });
    if (priorReferrer) initialRefUid = String(priorReferrer.user_uid || distributorUid);
  }

  const priorOwnerUid = String(existingLineProfile?.owner_uid || '').trim();
  if (priorOwnerUid) {
