...
async function createBooking(request, env) {
  ...
  const customerPhone = cleanPhone(body.customer_phone || body.customerPhone);
  ...
  if (!itineraryId || !customerName || !customerPhone) {
  ...
  const distributor = await resolveDistributor(env, tenantSlug, body);
  if (!distributor) {
  ...
  if (existingCustomer && String(existingCustomer.tenant_slug || 'demo') !== tenantSlug) {
    throw new Error('CUSTOMER_PHONE_TENANT_CONFLICT');
  }
  ...
  const orderInsert = env.DB.prepare(`
    INSERT INTO orders (
      ...
      customer_name, customer_phone, customer_line_uid, travelers, travel_date,
      note, status, commission_amount, total_amount, deposit_amount, balance_amount,
      payment_mode, balance_collect, deposit_status, deposit_paid_at, deposit_method,
      deposit_trade_no, balance_status, balance_paid_at, balance_method,
      balance_trade_no, commission_status, commission_settled_at,
      commission_paid_out_at, source, created_at, updated_at, tenant_slug
    ) VALUES (
      ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?, ?, ?,
      'unpaid', '', '', '', ?, '', '', '', 'pending', '', '', ?, ?, ?, ?
    )
  ...
  const customerUpsert = env.DB.prepare(`
    INSERT INTO customers (
      ...
      created_at, updated_at, tenant_slug
    ) VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?, '', ?, ?, ?)
  ...
  await customerUpsert.run();
  await env.DB.batch([orderInsert]);
...
async function createPayment(request, env, legacyWorker) {
  ...
  // 現有藍新金流參數仍是全平台共用。為避免 B 租戶誤用 demo 商店，
  // 非 demo 租戶在完成獨立金流設定表前一律安全阻擋。
  if (tenantSlug !== 'demo') {
