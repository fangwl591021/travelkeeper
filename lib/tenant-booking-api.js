...
async function prepareCustomerWrite(env, {
  ...
  if (existing) {
    ...
    const ownerUid = String(existing.owner_uid || '').trim() || distributorUid;
    const refUid = String(existing.ref_uid || '').trim() || distributorUid;
    return {
      customerId,
      customerKey,
      ownerUid,
...
async function createBooking(request, env) {
  ...
  const crmLink = env.DB.prepare(`
    ...
    SET customer_id = CASE WHEN customer_id = '' THEN ? ELSE customer_id END,
        ref_uid = CASE WHEN ref_uid = '' THEN ? ELSE ref_uid END,
        owner_uid = CASE WHEN owner_uid = '' THEN ? ELSE owner_uid END,
  ...
  // Keep customer and order writes first for legacy test/order semantics; CRM linking is a projection.
