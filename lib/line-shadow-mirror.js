...
export function shadowObservation(payload, env = {}, route = 'legacy', signatureValid = true) {
  const events = Array.isArray(payload?.events) ? payload.events : [];
  const config = shadowConfig(env);
  const allowlistMatched = events.some(event => isShadowUidAllowed(event?.source?.userId, env));
  return {
    route,
    signature_valid: Boolean(signatureValid),
    event_count: Math.min(events.length, SHADOW_EVENT_LIMIT),
    event_types: [...new Set(events.map(event => text(event?.type, 40)).filter(Boolean))],
    shadow_enabled: config.enabled,
    allowlist_matched: allowlistMatched,
    shadow_scheduled: Boolean(config.ready && allowlistMatched),
...
async function mirrorShadowEvents(events, env, fetchImpl = fetch, now = Date.now()) {
  const config = shadowConfig(env);
  if (!config.ready) return { mirrored: 0, skipped: 'disabled' };
  let mirrored = 0;
  let failed = 0;
  let duplicate = 0;
  for (const event of Array.isArray(events) ? events.slice(0, SHADOW_EVENT_LIMIT) : []) {
  ...
  return duplicate > 0 ? { mirrored, failed, duplicate } : { mirrored, failed };
}

export async function mirrorVerifiedWebhookPayload(payload, env, fetchImpl = fetch, now = Date.now()) {
  return mirrorShadowEvents(payload?.events, env, fetchImpl, now);
}

export async function mirrorVerifiedWebhookRequest(request, env, fetchImpl = fetch, now = Date.now()) {
  const body = JSON.parse(await request.text() || '{}');
  return mirrorVerifiedWebhookPayload(body, env, fetchImpl, now);
}
