...
function profileResponse(profile = {}, customer = {}, thread = {}, records = [], orders = []) {
  ...
  const ownerUid = customer.owner_uid || profile.owner_uid || '';
  const refUid = customer.ref_uid || profile.ref_uid || '';
...
async function loadCrm(request, env) {
  ...
  for (const row of customersResult.results || []) {
    const profile = {
      ...
      ref_uid: row.ref_uid || row.p_ref_uid || '', invite_code: row.p_invite_code || '',
      referral_note: row.p_referral_note || '', owner_uid: row.owner_uid || row.p_owner_uid || '',
...
async function saveProfile(request, env, forcedId = '') {
  ...
  const canonicalOwnerUid = text(customer?.owner_uid, 120);
  const canonicalRefUid = text(customer?.ref_uid, 120);
  if (customer && requestedOwnerUid !== undefined && text(requestedOwnerUid, 120) !== canonicalOwnerUid) {
    throw new Error('ATTRIBUTION_OWNER_CONFLICT');
  }
  if (customer && requestedRefUid !== undefined && text(requestedRefUid, 120) !== canonicalRefUid) {
    throw new Error('ATTRIBUTION_REFERRER_CONFLICT');
  }

  const ownerUid = customer
    ? canonicalOwnerUid
    : text(requestedOwnerUid ?? existing?.owner_uid ?? context.userUid, 120);
  if (!ownerAllowed(context, ownerUid)) throw new Error('CRM_PROFILE_ACCESS_DENIED');
  if (!customer && canManageAll(context) && requestedOwnerUid !== undefined) {
    await assertAssignableOwner(env, context.tenantSlug, requestedOwnerUid);
  }

  const refUid = customer
    ? canonicalRefUid
    : canManageAll(context)
      ? text(requestedRefUid ?? existing?.ref_uid ?? ownerUid, 120)
      : text(existing?.ref_uid || context.userUid, 120);
  if (!customer && !canManageAll(context) && requestedRefUid !== undefined && text(requestedRefUid, 120) && text(requestedRefUid, 120) !== refUid) {
    throw new Error('CRM_PROFILE_ACCESS_DENIED');
  }
  if (!customer && canManageAll(context) && requestedRefUid !== undefined) {
    await assertAssignableOwner(env, context.tenantSlug, requestedRefUid);
  }
