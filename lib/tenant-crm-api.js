...
async function saveProfile(request, env, forcedId = '') {
  ...
  const ownerUid = text(body.owner_uid ?? body.ownerUid ?? existing?.owner_uid ?? customer?.owner_uid ?? context.userUid, 120);
  ...
  const values = {
    ...
    ref_uid: text(body.ref_uid ?? body.refUid ?? existing?.ref_uid ?? ownerUid, 120),
