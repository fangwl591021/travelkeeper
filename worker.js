...
async function d1GetLineAudienceAnalysis(env) {
  if (!env.DB) throw new Error('D1 binding missing');
  await ensureLineVisitorRequirementsTable(env);
  const since7d = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
  const since30d = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString();

  const threadStats = await env.DB.prepare(`
    SELECT
      COUNT(*) AS total_threads,
      SUM(CASE WHEN status = 'open' THEN 1 ELSE 0 END) AS open_threads,
      SUM(CASE WHEN status = 'pending' THEN 1 ELSE 0 END) AS pending_threads,
  ...
  const requirementStats = await env.DB.prepare(`
    SELECT
      COUNT(*) AS total_requirements,
      SUM(CASE WHEN priority = 'high' THEN 1 ELSE 0 END) AS high_priority_requirements,
      SUM(CASE WHEN status = 'follow_up' THEN 1 ELSE 0 END) AS follow_up_requirements
    FROM line_visitor_requirements
    WHERE archived_at = ''
  `).first();
  ...
  const { results: topThreads } = await env.DB.prepare(`
    ...
    FROM line_threads t
    LEFT JOIN line_messages m ON m.thread_id = t.id
    GROUP BY t.id
    ORDER BY message_count DESC, COALESCE(t.last_message_at, t.created_at) DESC
    LIMIT 8
  `).all();
  ...
  const interestRules = [
    { key: 'japan', label: '日本/溫泉/賞雪', keywords: ['日本', '溫泉', '賞雪', '九州', '北海道', '東京', '大阪'] },
    { key: 'family', label: '親子/家庭', keywords: ['親子', '小孩', '家庭', '家族', '長輩', '兒童'] },
    { key: 'budget', label: '預算/費用', keywords: ['預算', '價格', '費用', '報價', '便宜', '刷卡'] },
    { key: 'schedule', label: '日期/天數', keywords: ['日期', '出發', '天數', '幾天', '假期', '連假'] },
  ...
  const { results: recentMessages } = await env.DB.prepare(`
    ...
    LIMIT 1000
  `).bind(since30d).all();
  const interestCounts = interestRules.map(rule => ({
    key: rule.key,
    label: rule.label,
    count: (recentMessages || []).filter(row => {
  ...
  return {
    ...
    data: {
      ...
      overview: {
        ...
        openThreads: Number(threadStats?.open_threads || 0),
        pendingThreads: Number(threadStats?.pending_threads || 0),
        closedThreads: Number(threadStats?.closed_threads || 0),
        highRiskThreads: Number(threadStats?.high_risk_threads || 0),
        mediumRiskThreads: Number(threadStats?.medium_risk_threads || 0),
        activeThreads7d: Number(threadStats?.active_threads_7d || 0),
        activeThreads30d: Number(threadStats?.active_threads_30d || 0),
        unreadMessages: Number(threadStats?.unread_messages || 0),
        totalMessages: Number(messageStats?.total_messages || 0),
        messages7d: Number(messageStats?.messages_7d || 0),
        messages30d: Number(messageStats?.messages_30d || 0),
        totalRequirements: Number(requirementStats?.total_requirements || 0),
        highPriorityRequirements: Number(requirementStats?.high_priority_requirements || 0),
        followUpRequirements: Number(requirementStats?.follow_up_requirements || 0),
      },
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (path === '/api/line-oa/audience' && request.method === 'GET') {
        const uid = url.searchParams.get('uid') || '';
        ...
        const result = await d1GetLineAudienceAnalysis(env);
        return json(result);
      }
