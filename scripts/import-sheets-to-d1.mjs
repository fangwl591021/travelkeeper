...
function buildSql(dataset, { truncate = true } = {}) {
  ...
  for (const it of dataset.itineraries) {
    lines.push(
      ...
      `id, title, region, price, days, image, description, notes, owner_uid, owner_name, review_status, review_note, payment_mode, deposit_ratio, balance_collect, created_at, updated_at, deleted_at` +
      `) VALUES (` +
      `${sqlText(it.id)}, ${sqlText(it.title)}, ${sqlText(it.region)}, ${sqlInt(it.price)}, ${sqlInt(it.days)}, ${sqlText(it.image)}, ${sqlText(it.description)}, ${sqlText(it.notes)}, ${sqlText(it.owner_uid)}, ${sqlText(it.owner_name)}, ${sqlText(it.review_status)}, ${sqlText(it.review_note)}, ${sqlText(it.payment_mode)}, ${sqlInt(it.deposit_ratio, 20)}, ${sqlText(it.balance_collect)}, ${sqlText(it.created_at)}, ${sqlText(it.updated_at)}, ${sqlText(it.deleted_at)}` +
      `);`
    );
  }
