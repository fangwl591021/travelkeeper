...
class CompatDb {
  constructor() { this.lastAllArgs = null; }
  ...
  async first(sql, args) {
    ...
    if (sql.includes('FROM customers')) {
      return {
        ...
        owner_uid: 'U-SALES', owner_name: 'Sales', total_orders: 2, total_amount: 24000,
        source: 'referral', created_at: '2026-07-11 10:00:00', updated_at: '2026-07-11 10:00:00',
      };
    }
    return null;
  }
...
test('only explicit non-demo legacy requests enter the compatibility layer', () => {
  ...
  assert.equal(isLegacyCustomerCompatRequest(new Request('https://worker.example/api/my/customers', { headers: { Referer: 'https://example.com/dashboard.html?tenant=partner-a' } })), true);
  assert.equal(isLegacyCustomerCompatRequest(new Request('https://worker.example/api/my/customers', { headers: { Referer: 'https://example.com/dashboard.html?tenant=demo' } })), false);
  assert.equal(isLegacyCustomerCompatRequest(new Request('https://worker.example/api/my/customers', { headers: { Referer: 'https://example.com/dashboard.html?tenant=../bad' } })), false);
...
test('legacy views display contact phone while retaining a separate relation key', () => {
  const customer = toLegacyCustomerView({ customer_id: 'CUS1', customer_phone: 'CUSREL1', contact_phone: '0912000000', total_orders: 2 });
  ...
  const order = toLegacyOrderView({ order_id: 'ORD1', customer_id: 'CUS1', customer_phone: 'CUSREL1', contact_phone: '0912000000' });
...
test('mother customer and order payloads use customer id and contact phone', () => {
  const customer = buildTenantCustomerExportPayload({ tenant_slug: 'partner-a', customer_id: 'CUS1', customer_phone: 'CUSREL1', contact_phone: '0912000000' });
  assert.equal(customer.local_id, 'CUS1');
  assert.equal(customer.customer_id, 'CUS1');
  assert.equal(customer.customer_phone, '0912000000');
  assert.equal('customer_key' in customer, false);
  const order = buildTenantOrderExportPayload({ tenant_slug: 'partner-a', order_id: 'ORD1', customer_id: 'CUS1', customer_phone: 'CUSREL1', contact_phone: '0912000000' });
...
test('tenant customer mother export dry run is tenant scoped', async () => {
  const request = new Request('https://worker.example/api/mother/export-customer?tenant=partner-a', {
    method: 'POST', headers: { 'Content-Type': 'application/json', 'X-Tenant-Slug': 'partner-a', 'X-User-Uid': 'U-ADMIN' },
